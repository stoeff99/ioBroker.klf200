...
import {
	...
	createTlsConnectionOptionsWithFingerprintPinning,
...
export class Klf200 extends utils.Adapter implements HasConnectionInterface, HasProductsInterface, Translate {
	...
	private async onReady(): Promise<void> {
		try {
			...
			const sslPublicKey =
				this.config.advancedSSLConfiguration && this.config.SSLPublicKey.trim() !== ""
					? Buffer.from(this.config.SSLPublicKey)
					: undefined;
			const sslFingerprint =
				this.config.advancedSSLConfiguration && this.config.SSLFingerprint.trim() !== ""
			...
			configureConnectionWithTlsFingerprintPinning(
				...
				this.config.SSLConnectionOptions,
				sslPublicKey,
				sslFingerprint,
			);
	...
	private createConnectionOptions(data: ConnectionTestMessage): ConnectionOptions {
		const publicKey =
			...
			data.advancedSSLConfiguration.sslPublicKey.trim() !== ""
				...
				: undefined;
		const fingerprint =
			data.advancedSSLConfiguration?.sslFingerprint !== undefined &&
		...
		return createTlsConnectionOptionsWithFingerprintPinning(
			klf200Connection.fingerprint,
			undefined,
			klf200Connection.CA,
		);
	}
