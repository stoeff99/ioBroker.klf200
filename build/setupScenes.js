"use strict";
import { ComplexPropertyChangedHandler, ComplexStateChangeHandler, EchoStateChangeHandler, MethodCallStateChangeHandler, } from "./util/propertyLink.js";
...
export class SetupScenes {
     ...
     */
    static async createScenesAsync(adapter, scenes, disposalMap) {
        // Remove old scenes
        ...
        const channelsToRemove = currentScenesList.filter(channel => !scenes.Scenes.some(scene => {
        ...
        await StateHelper.createAndSetStateAsync(adapter, `scenes.scenesFound`, {
        ...
        await StateHelper.createAndSetStateAsync(adapter, `scenes.refreshScenes`, {
        ...
        const refreshScenesChangeHandler = new MethodCallStateChangeHandler(adapter, "scenes.refreshScenes", scenes, "refreshScenesAsync");
     ...
     */
    static async createSceneAsync(adapter, scene, disposalMap) {
        ...
        await StateHelper.createAndSetStateAsync(adapter, `scenes.${scene.SceneID}.productsCount`, {
            name: "productsCount",
            role: "value",
            type: "number",
            read: true,
        ...
        await StateHelper.createAndSetStateAsync(adapter, `scenes.${scene.SceneID}.products`, {
            name: "products",
            role: "value",
            type: "array",
            read: true,
        ...
        await StateHelper.createAndSetStateAsync(adapter, `scenes.${scene.SceneID}.run`, {
            name: "run",
            role: "button.play",
            type: "boolean",
            read: true,
        ...
        await StateHelper.createAndSetStateAsync(adapter, `scenes.${scene.SceneID}.stop`, {
            name: "stop",
            role: "button.stop",
            type: "boolean",
            read: false,
        ...
        disposalMap.set(`scenes.${scene.SceneID}.property.IsRunning`, new ComplexPropertyChangedHandler(adapter, "IsRunning", scene, async (newValue) => {
        ...
        disposalMap.set(`scenes.${scene.SceneID}.property.Products`, new ComplexPropertyChangedHandler(adapter, "Products", scene, async (newValue) => {
        ...
        const runListener = new ComplexStateChangeHandler(adapter, runStateId, async (state) => {
            if (state !== undefined) {
                if (state?.val === true) {
                    ...
                    if (!scene.IsRunning) {
                        // Get the velocity
                        const velocity = (await adapter.getStateAsync(`scenes.${scene.SceneID}.velocity`))?.val;
                        // Run the scene
