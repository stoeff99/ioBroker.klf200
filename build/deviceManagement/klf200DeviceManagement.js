 ...
 */
export class KLF200DeviceManagement extends DeviceManagement {
    async listDevices() {
    ...
    async getInstanceInfo() {
        ...
        this.adapter.log.debug(`KLF200DeviceManagement: getInstanceInfo: Returning ${JSON.stringify(instanceDetails)}.`);
    ...
    async handleInstanceAction(actionId, context) {
        switch (actionId) {
            ...
            default:
                throw new Error(`Instance action ${actionId} is unknown.`);
        }
    }
    async handleDeviceAction(deviceId, actionId, context) {
        switch (actionId) {
            ...
            default:
                throw new Error(`Device action ${actionId} is unknown.`);
        }
    }
    async handleInstanceDiscover(context) {
        // Start discovery
        const progressDialog = await context?.openProgress(await this.adapter.translate("dm-instance-discover-progress-title"), { indeterminate: true });
        try {
            try {
                ...
                await context?.showMessage(`${await this.adapter.translate("dm-instance-discover-progress-error")}\n${this.getErrorMessage(error)}.`);
            }
            // Return the result
            return { refresh: true };
        }
    ...
    async handleSendToRemote(context) {
        assert(context, "handleSendToRemote: Action context shouldn't be null or undefined.");
        await context.showMessage("Not implemented.");
        return { refresh: false };
    }
    async handleReceiveFromRemote(context) {
        assert(context, "handleReceiveFromRemote: Action context shouldn't be null or undefined.");
        await context.showMessage("Not implemented.");
        return { refresh: false };
    }
    async handleProductDelete(deviceId, context) {
        const productId = parseInt(deviceId.split(".").reverse()[0]);
        const confirmationDialog = await context?.showConfirmation(await this.adapter.translate("dm-device-product-delete-confirm", { productId: productId.toString() }));
        if (confirmationDialog) {
            try {
                await this.adapter.onRemoveProduct(productId);
            }
            catch (error) {
                await context?.showMessage(`${await this.adapter.translate("dm-device-product-delete-error")}\n${this.getErrorMessage(error)}.`);
            }
            return Promise.resolve({ refresh: true });
        }
        return Promise.resolve({ refresh: false });
    }
    async handleProductRename(deviceId, context) {
        const productId = parseInt(deviceId.split(".").reverse()[0]);
        ...
        const newName = await this.showRenameForm(context, await this.adapter.getTranslatedObject("dm-device-product-rename-form-title"), await this.adapter.getTranslatedObject("dm-device-product-rename-form-label"), oldName);
        if (newName !== undefined) {
            try {
                await this.adapter.onRenameProduct(productId, newName);
            }
            catch (error) {
                await context?.showMessage(`${await this.adapter.translate("dm-device-product-rename-error")}\n${this.getErrorMessage(error)}.`);
            }
            return { refresh: true };
        }
        return { refresh: false };
    }
    async handleProductWink(deviceId, context) {
        const productId = parseInt(deviceId.split(".").reverse()[0]);
        try {
            await this.adapter.onWinkProduct(productId);
        }
        catch (error) {
            await context?.showMessage(`${await this.adapter.translate("dm-device-product-wink-error")}\n${this.getErrorMessage(error)}.`);
        }
        return Promise.resolve({ refresh: false });
    }
    async showGroupEditDialog(context, groupEditDialogData) {
        const form = {
            type: "panel",
            items: {
                groupId: {
                    ...
                    placeholder: await this.adapter.getTranslatedObject("dm-device-group-edit-form-groupId-placeholder"),
                },
                groupName: {
                    type: "text",
                    label: "Group name",
                    validator: "data.groupName !== undefined && data.groupName !== ''",
                    validatorErrorText: await this.adapter.translate("dm-device-group-edit-form-groupName-validatorErrorText"),
        ...
        const result = {
            groupId: groupEditDialogData.groupId,
            groupName: resultFormData.groupName,
            products: [],
        };
        if (this.adapter.Products) {
            for (const product of this.adapter.Products.Products) {
    ...
    async handleAddGroup(context) {
        ...
        try {
            ...
            await context?.showMessage(`${await this.adapter.translate("dm-instance-creategroup-error")}\n${this.getErrorMessage(error)}.`);
        }
        return { refresh: true };
    }
    async handleEditGroup(deviceId, context) {
        ...
        try {
            ...
            await context?.showMessage(`${await this.adapter.translate("dm-device-group-edit-error")}\n${this.getErrorMessage(error)}.`);
        }
        return { refresh: true };
    }
    async handleAddScene(context) {
        ...
        const differentNameCondition = `[${this.adapter.Scenes?.Scenes.filter(scene => scene !== undefined)
            ...
            .join(",")}]`;
        const newSceneName = await this.showRenameForm(context, await this.adapter.getTranslatedObject("dm-instance-createscene-form-title"), await this.adapter.getTranslatedObject("dm-instance-createscene-form-label"), "", `data.name !== undefined && data.name.trim() !== ''${differentNameCondition === "" ? "" : ` && !${differentNameCondition}.includes(data.name)`}`, differentNameCondition === ""
            ? undefined
        ...
        if (newSceneName !== undefined) {
            ...
            try {
                ...
                await dlg?.close();
                dlg = undefined;
                const confirmationDialog = await context.showConfirmation(failedNodes.length === 0
                    ? await this.adapter.getTranslatedObject("dm-instance-createscene-progress-use-remote")
                    : await this.adapter.getTranslatedObject("dm-instance-createscene-progress-use-remote-with-failed-nodes", { failedNodes: failedNodes.join(", ") }));
                if (!confirmationDialog) {
                ...
                await dlg?.close();
                dlg = undefined;
                await context?.showMessage(`${await this.adapter.translate("dm-instance-createscene-error")}\n${this.getErrorMessage(error)}.`);
                return { refresh: true };
            }
        }
        return { refresh: false };
    }
    async handleGroupDelete(deviceId, context) {
        const groupId = parseInt(deviceId.split(".").reverse()[0]);
        const confirmationDialog = await context?.showConfirmation(await this.adapter.getTranslatedObject("dm-device-group-delete-confirm", { groupId: groupId.toString() }));
        if (confirmationDialog) {
            try {
                await this.adapter.onRemoveGroup(groupId);
            }
            catch (error) {
                await context?.showMessage(`${await this.adapter.translate("dm-device-group-delete-error")}\n${this.getErrorMessage(error)}.`);
            }
            return Promise.resolve({ refresh: true });
        }
        return Promise.resolve({ refresh: false });
    }
    async handleSceneDelete(deviceId, context) {
        const sceneId = parseInt(deviceId.split(".").reverse()[0]);
        const confirmationDialog = await context?.showConfirmation(await this.adapter.getTranslatedObject("dm-device-scene-delete-confirm", { sceneId: sceneId.toString() }));
        if (confirmationDialog) {
            try {
                await this.adapter.onRemoveScene(sceneId);
            }
            catch (error) {
                await context?.showMessage(`${await this.adapter.translate("dm-device-scene-delete-error")}\n${this.getErrorMessage(error)}.`);
            }
            return Promise.resolve({ refresh: true });
        }
        return Promise.resolve({ refresh: false });
    }
    async handleSceneRename(deviceId, context) {
        const sceneId = parseInt(deviceId.split(".").reverse()[0]);
        ...
        const newName = await this.showRenameForm(context, await this.adapter.getTranslatedObject("dm-device-scene-rename-form-title"), await this.adapter.getTranslatedObject("dm-device-scene-rename-form-label"), oldName);
        if (newName !== undefined) {
            try {
                await this.adapter.onRenameScene(sceneId, newName);
            }
            catch (error) {
                await context?.showMessage(`${await this.adapter.translate("dm-device-scene-rename-error")}\n${this.getErrorMessage(error)}.`);
            }
            return { refresh: true };
        }
        return { refresh: false };
    }
    async showRenameForm(context, title, label, oldValue, validator, validatorErrorText) {
        const formSchema = {
            type: "panel",
            items: {
                name: {
...
//# sourceMappingURL=klf200DeviceManagement.js.map